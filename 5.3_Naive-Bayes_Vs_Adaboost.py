...
# DESC: Experiment 2) Compare these models A) Naive Bayes Multinomial B) OneVsRest(Naive Bayes Multinomial), C) Adaboost with Naive Bayes Multinomial as base models
...
for counter in range(1, 4, 1):
	...
	for f in os.listdir(data_folder):
		...
		if f.startswith("2_"):
			...
			if counter == 1:
				my_model = naive_bayes.MultinomialNB(alpha=alpha_val)
				...
				my_model = ensemble.AdaBoostClassifier(
					model=(
						multiclass.OneVsRestClassifier(naive_bayes.MultinomialNB(alpha=alpha_val))
					),
					n_models=10,
					seed=42
				)
			...
			for i, row in data.iterrows():
				...
				y_pred = my_model.predict_one(x)
				my_model.learn_one(x, y)
	...
	if counter == 1:
		...
		bars1 = ax.bar(x - width, metric_table, width, color="purple", label='NBM', zorder=2)
	if counter == 2:
		ax.axhline(y=avg, color='orange', linestyle='--', linewidth=2, zorder=1)
		bars2 = ax.bar(x , metric_table, width, color="orange",  label='Binary_NBM', zorder=2)
	if counter == 3:
		ax.axhline(y=avg, color='blue', linestyle='--', linewidth=2, zorder=1)
		bars3 = ax.bar(x + width, metric_table, width, color="blue", label='Adaboost_Binary_NBM', zorder=2)
	...
	ax.set_title('Comparison of NBM Vs Binary-NBM Vs Adaboost-Binary-NMB ', fontsize=24, fontweight='bold')
	ax.set_xticks(x)
...
plt.savefig(os.path.abspath(graphs_folder) + "/" + "5.2_Naive-Bayes_Vs_Adaboost.png")
plt.show()
